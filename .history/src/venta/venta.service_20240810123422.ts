...
export class VentaService {
  ...
  async generarPdf(saleId: string): Promise<Buffer> {
    const pdfBuffer: Buffer = await new Promise(async (resolvePromise) => {
      ...
      // Obtener la venta por ID y los pagos mensuales asociados
      ...
      const creditData = [
        ...
        ...sale.monthlyPayments.map((payment) => [
          payment.amount.toFixed(2), // Formato a 2 decimales
          payment.dueDate.toLocaleDateString(), // Formato de fecha local
        ]),
      ];
