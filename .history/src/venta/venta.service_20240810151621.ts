  ...
  async generarPdf(saleId: string): Promise<Buffer> {
    const pdfBuffer: Buffer = await new Promise(async (resolvePromise) => {
      ...
      doc
        ...
        // .text('EMPRESA INMOBILIARIA PALOMINO', 50, 10, { align: 'left' });


      doc.fontSize(16).text(' EMPRESA INMOBILIARIA PALOMINO S.A.C', { align: 'center' });
      doc.moveDown();
      ...
      doc
        ...
        .text(`Nombre del Cliente:`, { align: 'left',continued:true },)
        .font('Helvetica')
        .text(`${sale.client.fullName}`,{align:'top-right'})
