  ...
  async generarPdf(saleId: string): Promise<Buffer> {
    const pdfBuffer: Buffer = await new Promise(async (resolvePromise) => {
      ...
      doc.font('Helvetica-Bold')
        ...
        .text(`Nombre del Cliente: ${sale.client.name}`, { align: 'left' })
        .text(`Lote: ${sale.lot.name}`, { align: 'left' })
      ...
      const creditData = [
        ...
        ...sale.monthlyPayments.map((payment, index) => [
          ...
          payment.amount.toFixed(2), // Monto
          payment.dueDate.toLocaleDateString(), // Fecha de Pago
        ]),
      ];
      ...
      doc.table({
        headers: creditData[0],
        rows: creditData.slice(1),
        columnWidth: [40, 100, 120], // Ajustar el ancho de las columnas
        align: ['center', 'right', 'left'],
        padding: 5,
        margin: { top: 15 },
        headerStyle: {
